        ...
        div class="p-6 md:p-8 bg-gradient-to-b from-neutral-900 to-neutral-900/40" {
            div class="flex flex-wrap items-center gap-2 mb-3" {
                ...
                @if let Some(domain) = &detail.domain {
                    span class="px-2.5 py-1 rounded-full text-xs font-medium bg-neutral-800 text-neutral-300 border border-neutral-700/60 font-mono flex items-center gap-1" {
                        svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3 text-neutral-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" {
            ...
            h1 class="text-xl md:text-2xl font-bold text-neutral-100 leading-snug tracking-tight mb-4" {
                @if let Some(url) = &detail.url {
                    ...
                    {
                        span { (detail.title) }
                        svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-neutral-400 group-hover:text-amber-400 mt-1 flex-shrink-0 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" {
...
pub fn story_actions(detail: &Story) -> Markup {
    html! {
        div class="flex flex-wrap items-center gap-2.5 pt-2" {
            ...
            {
                svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5 text-amber-500" viewBox="0 0 24 24" fill="currentColor" {
            ...
            button
                ...
                class="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-medium bg-neutral-800/80 hover:bg-neutral-700 text-neutral-300 border border-neutral-700/60 active:scale-95 transition-all ml-auto"
            {
                svg id="refresh-item-icon" xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5 text-neutral-400 group-hover:rotate-180 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" {
                    path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" {}
                }
                span id="refresh-item-spinner" class="htmx-indicator animate-spin inline-block w-3.5 h-3.5 border-2 border-amber-500 border-t-transparent rounded-full" {}
