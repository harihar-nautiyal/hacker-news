    ...
    html! {
            ...
            onclick=(format!("selectStoryCard('{}'); showMobileDetail();", story.id))
        {
            div class="flex items-start gap-3" {
                // Rank Number
                span class="text-xs font-mono font-semibold text-neutral-400 w-5 pt-0.5 text-right flex-shrink-0 group-hover:text-amber-500 transition-colors" {
                    (story.rank)
                }
                ...
                div class="flex-1 min-w-0" {
                    ...
                    div class="flex flex-wrap items-baseline gap-x-2 gap-y-1 mb-1.5" {
                        ...
                        @if let Some(domain) = &story.domain {
                            span class="inline-flex items-center text-[11px] font-mono text-neutral-400 hover:text-neutral-300 transition-colors" onclick="event.stopPropagation()" {
                                @if let Some(url) = &story.url {
                                    a href=(url) target="_blank" rel="noopener noreferrer" class="hover:underline flex items-center gap-0.5" title="Open external link" {
                                        span { "(" (domain) ")" }
                    ...
                    // Metadata Pills
                    div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-neutral-400" {
                        // Points
                        span class="inline-flex items-center gap-1 font-mono font-semibold text-amber-500/90 text-[11px]" {
                        ...
                        // Author
                        span class="truncate hover:text-neutral-300" {
                            "by @" (story.author)
                        }
                        ...
                        a
                            ...
                            class="inline-flex items-center gap-1 hover:text-neutral-300 font-mono text-[11px] text-neutral-400"
